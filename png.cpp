...
{   
    ...
    uint32_t crc;
    crc=net_to_host(*reinterpret_cast<uint32_t*>(PNG_swap.swap_buffer+offset));
    offset+=sizeof(uint32_t);
    if(crc!=calc_crc32(PNG_swap.swap_buffer+PNG_SIGNATURE_LENGTH, CHUNK_COMPULSORY_LENGTH-4+IHDR_DATA_LENGTH)) {
        SET_ERROR(status, PNGErrorCode::DEFAULT_ERROR, IOErrorCode::DEFAULT_ERROR, "CRC check failed for IHDR chunk");
        return false;
    }
